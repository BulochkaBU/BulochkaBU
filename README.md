...
Вся работа будет проводиться в вашем репозитории (этом). Давайте подробнее разберемся, что здесь есть и как с этим работать.
...
Всего в проекте 14 модулей. Сейчас вы можете увидеть задачи на первую неделю (т.е. 2 первых milestones или модуля), каждый понедельник у вас в репозитории будут **автоматически** появляться задачи на новую неделю.
...
1. В описании (поле description) PR написать "Closes #<номер issue>" (обязательно пробел перед знаком решетки), например, `Closes #2` или `Closes #5`. **Обратите внимание: указывать нужно номер issue, а не номер label**. Номер issue можно всегда посмотреть в списке issues. Альтернативный вариант: выбрать нужный issue в меню справа в разделе Linked issues 
